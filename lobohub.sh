...
then
  ...
  then
    ...
    read MAXC
    echo ""
    echo "Enter Swap size in MB: (For 2G enter 2048. If you dont want to create enter 0"
    read SWAP
  fi
  if [ $DOSETUP = "y" ]
  then
    ...
    if [[ ${SWAP} == 0 ]]
    then
      echo "Skipped creating swap."
    else
      echo -e "${RED}Creating swap. This may take a while.${NC}"
      dd if=/dev/zero of=/var/swap.img bs=${SWAP} count=1M
